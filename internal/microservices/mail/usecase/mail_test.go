...
import (
	...
	mockUserRepo "github.com/go-park-mail-ru/2023_1_Seekers/internal/microservices/user/usecase/mocks"
...
func TestUseCase_GetFolders(t *testing.T) {
	...
	userRepo := mockUserRepo.NewMockUseCaseI(ctrl)
	mailH := New(cfg, mailRepo, userRepo)
...
func TestUseCase_GetFolderMessages(t *testing.T) {
	...
	userRepo.EXPECT().GetInfo(mockFolderMessagesResponse[0].FromUser.UserID).Return(&mockUserResponse[0], nil)
	...
	userRepo.EXPECT().GetInfo(mockRecipientsResponse[0]).Return(&mockUserResponse[1], nil)
...
func TestUseCase_GetMessage(t *testing.T) {
	...
	userRepo.EXPECT().GetInfo(mockUserResponse[0].UserID).Return(&mockUserResponse[0], nil)
...
func TestUseCase_SendMessage(t *testing.T) {
	...
	userRepo.EXPECT().GetInfo(userID).Return(&mockUserResponse[0], nil)
	mailRepo.EXPECT().SelectRecipientsByMessage(messageSelected.MessageID, userID).Return([]uint64{mockUserResponse[1].UserID}, nil)
	userRepo.EXPECT().GetInfo(mockUserResponse[1].UserID).Return(&mockUserResponse[0], nil)
...
func TestUseCase_SendFailedSendingMessage(t *testing.T) {
	...
	userRepo.EXPECT().GetInfo(mockUserResponse[1].UserID).Return(&mockUserResponse[1], nil)
