...
func RegisterHTTPRoutes(r *mux.Router, cfg *config.Config, authH AuthHandlersI, userH UserHandlersI, mailH MailHandlersI, m *middleware.HttpMiddleware) {
	...
	r.HandleFunc(cfg.Routes.RoutePrefix+cfg.Routes.RouteFilterMessages, m.CheckAuth(m.CheckCSRF(mailH.FilterMessages))).Methods(http.MethodGet)
	...
	r.HandleFunc(cfg.Routes.RoutePrefix+cfg.Routes.RouteAttachB64, m.CheckAuth(m.CheckCSRF(mailH.GetAttachB64))).Methods(http.MethodGet)
	r.HandleFunc(cfg.Routes.RoutePrefix+cfg.Routes.RouteMessageAttaches, m.CheckAuth(m.CheckCSRF(mailH.DownloadAllAttaches))).Methods(http.MethodGet)
